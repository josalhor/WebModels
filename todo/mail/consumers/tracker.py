...
def parse_references(book, references):
    related_messages = []
    answer_thread = None
    for related_message in references.split():
        ...
        new_answer_thread = Task.objects.filter(book=book, pk=thread_id).first()
...
def insert_message(book, message, priority, task_title_format):
    ...
    related_messages, answer_thread = parse_references(book, message.get("references", ""))
    ...
    best_task = (
        Task.objects.filter(book=book, comment__email_message_id__in=related_messages)
...
def tracker_consumer(
    producer, group=None, book_slug=None, priority=1, task_title_format="[MAIL] {subject}"
):
    book = Book.objects.get(slug=book_slug)
    for message in producer:
        try:
            insert_message(book, message, priority, task_title_format)
        except Exception:
