...
def deactivate_user(request):
    ...
    if request.method == "POST":
        User = get_user_model()
        user = User.objects.filter(email=request.POST['deactivate-user']).first()
        ...
        messages.success(request, "El usuario '{}' ha sido desactivado correctamente.".format(user.full_name))
