...
def deactivate_book(request):
    ...
    if request.method == "POST":
        book = PublishedBook.objects.filter(pk=request.POST['deactivate-book']).first()
        book.disabled = True

        book.save()

        messages.success(request, "El libro '{}' ha sido desactivado correctamente.".format(book.title))
