...
def del_list(request, list_id: int, list_slug: str) -> HttpResponse:
    ...
    if book_list.group not in request.user.groups.all():
        raise PermissionDenied    
    if not request.user.is_staff:
        raise PermissionDenied

    if request.method == "POST":
        Book.objects.get(id=book_list.id).delete()
        messages.success(request, "{list_name} is gone.".format(list_name=book_list.name))
        return redirect("todo:lists")
    else:
        task_count_done = Task.objects.filter(book_list=book_list.id, completed=True).count()
        task_count_undone = Task.objects.filter(book_list=book_list.id, completed=False).count()
        task_count_total = Task.objects.filter(book_list=book_list.id).count()
